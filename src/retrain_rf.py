...
def retrain_model(base_model, X_train, y_train, X_test):
    if base_model is not None:
        ...
        if hasattr(model, 'n_estimators'):
            ...
            print(f"Extending model from {original_n_estimators} to {model.n_estimators} estimators")
    else:
        print("Training new Random Forest model from scratch...")
...
def main():
    ...
    # Start MLflow run
    with start_run(run_name="random_forest_retraining"):
        ...
        results_default, results_fixed, fixed_threshold, report_default, report_fixed = evaluate_model(
            "Random Forest (Retrained)", y_test, y_test_pred, y_test_prob, train_time, prediction_time, FIXED_FPR
        )
        ...
        # Save model bundle
        model_path = save_model(model, "Random Forest Classifier (Retrained)", 
                                fixed_threshold, FIXED_FPR, metadata, "rf_model_bundle_retrained.pkl")
        ...
        # Log model to MLflow
        log_model_and_params(
            model, "Random Forest Classifier (Retrained)", metadata, model_path,
        ...
        save_training_report(results_default, results_fixed, report_default, report_fixed, 
                            metadata, "training_report_rf_retrained.json")

        # Log report to MLflow
        log_training_report(ARTIFACTS_DIR / "training_report_rf_retrained.json")
